...
def train_and_save_cnn(x_train, y_train, x_val, y_val):
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv2D(32, (3, 3), activation='relu'), # Couche convolutive (32 filtres)
        tf.keras.layers.MaxPooling2D(2, 2), # Couche réduction taille image + réduction surapprentissage (pooling)
        tf.keras.layers.Flatten(), # transformer matrice en ligne (on aplatit l'image)
        tf.keras.layers.Dense(128, activation='relu'), # Couche dense avec 128 neurones qui a la fonction d'activation relu (f(x)=max(0,x) 
...
#    random_search_rf.fit(x_train.reshape(-1, 28*28), y_train)  # Aplatir les images en vecteurs pour l'entraînement RF
    # Sauvegarde du meilleur modèle Random Forest trouvé
#   joblib.dump(random_search_rf.best_estimator_, MODEL_RF_PATH)
#   print(f"RandomForest Model saved to {MODEL_RF_PATH}")  # Confirmation que le modèle a été enregistré
